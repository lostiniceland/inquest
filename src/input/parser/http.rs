...
fn parse_get(hocon: &Hocon) -> Result<Http> {
    let url = hocon["url"].as_string().ok_or(InquestError::ConfigurationError)?;
...
mod tests {
    ...
    fn parse_http() {
        ...
        match_content(content, |config| match config {
            Config::Http(Http {  url, status, name, .. }) => {
                assert_eq!("https://httpbin.org/get", url.to_string());
                assert_eq!(200, *status);
                assert_eq!("Testing GET against HTTPBin", name.deref().as_ref().unwrap());
            }
